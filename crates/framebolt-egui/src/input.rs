...
use crate::{panels::{Platform, canvas::CanvasInteractionState}, renderer::SharedRenderer};
...
fn handle_desktop_canvas_input(
    ...
    apply_camera_command_if_needed(
        renderer,
        cmd,
        viewport,
    );
...
fn handle_mobile_canvas_input(
    ...
    if let Some(multi_touch) =
        ui.input(|i| i.multi_touch())
    {
        let mut cmd =
        ...
        if let Some(last_center) =
            interaction.last_touch_center
        {
            let delta =
                center - last_center;
            ...
            cmd.pan = glam::vec2(
                delta.x,
                -delta.y,
            ) * 2.0; // todo figure out why this is needed for panning to feel close to actually pinned
        }
...
fn apply_camera_command_if_needed(
    ...
    {
        renderer.lock().apply_camera_command(
            cmd,
            viewport.x,
            viewport.y,
        );
