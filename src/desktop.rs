...
pub fn create_dock_state() -> DockState<DockTab> {
    let mut dock_state = DockState::new(vec![DockTab {
        ...
        egui::MenuBar::new().ui(ui, |ui| {
            ui.menu_button("File", |ui| {
                ...
                ui.menu_button("Open Recent", |ui| {
                    let _ = ui.button("~/Documents/framebolt/not-a-real-project");

                    let _ = ui.button("~/Documents/framebolt/also-not-a-real-project");

                    let _ = ui.button("~/Documents/framebolt/hentai");

                    let _ = ui.button("~/Documents/framebolt/idk");
                });
...
fn menu_item_with_shortcut(ui: &mut egui::Ui, label: &str, shortcut: &str) -> egui::Response {
...
fn menu_item_width(ui: &egui::Ui, label: &str, shortcut: &str) -> f32 {
    let font_id = egui::TextStyle::Button.resolve(ui.style());
    ...
    let shortcut_width = if shortcut.is_empty() {
        ...
        ui.painter()
            .layout_no_wrap(shortcut.to_owned(), font_id, ui.visuals().text_color())
            .size()
            .x
    };
...
impl<'a> egui_dock::TabViewer for TabViewer<'a> {
    ...
    fn ui(&mut self, ui: &mut egui::Ui, tab: &mut DockTab) {
        if let Some(panel) = self.panels.panel_mut(tab.panel) {
